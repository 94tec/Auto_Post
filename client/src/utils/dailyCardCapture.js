...
export const captureDailyCardAsBlob = async (ref) => {
  ...
  try {
    blob = await domtoimage.toBlob(node, {
      ...
      //bgcolor: 'linear-gradient(165deg,#141E2E,#0A0E1A,#0D1520)',
    });
  } finally {
...
export const downloadDailyCard = async (ref, filename = 'damuchi-daily.png') => {
  const blob = await captureDailyCardAsBlob(ref);
...
export const previewAndPostDailyCard = async (ref, { text, url, item, sourceType = 'lyric' }) => {
  const blob = await captureDailyCardAsBlob(ref);
  ...
  // Expose poster on opener — auth stays in main window
...
<head>
  ...
  <title>Share — Damuchi Daily</title>
  <style>
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .card-wrap {
      ...
      box-shadow: 0 32px 96px rgba(0,0,0,.8), 0 0 0 1px rgba(255,255,255,.08);
    }
...
<body>
  <p class="label-top">Your daily card preview</p>

  <div class="card-wrap">
    <img id="cardImg" src="${base64}" alt="Daily card"/>
  </div>
  ...
  <div class="actions">
    ...
    <a class="btn btn-dl" href="${base64}" download="damuchi-daily.png">
      ↓ &nbsp; Download
    </a>
  </div>
  ...
  <p class="label-bottom">
    Posted directly to your connected X account with the card image attached.
  </p>
  ...
  <script>
    ...
    const SOURCE_TYPE = ${JSON.stringify(sourceType ?? 'lyric')};
    ...
    async function postToX() {
      ...
      try {
        ...
        if (!poster) throw new Error('Connection lost — close and try again');
