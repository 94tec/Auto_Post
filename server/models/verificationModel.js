...
class VerificationModel {
  constructor() {
    this.verificationsRef = db.ref('verifications');
    this.verificationsByUserIndexRef = db.ref('indexes/verifications_by_user');
  }

  async create(data) {
    try {
      // 1. Validate required fields
      const { userId, token, email, type = 'email_verification' } = data;
      ...
      if (snapshot.exists()) {
        ...
        if (existingActive) {
          ...
          return {
            ...record,
            _path: `/verifications/${existingId}`,
      ...
      const updates = {};
      updates[`/verifications/${id}`] = record;
      updates[`/indexes/verifications_by_user/${userId}/${id}`] = true; // Index entry
